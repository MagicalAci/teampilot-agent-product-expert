...
""",
    ...
    def test_bootstrap_standalone_smoke_from_copied_skill_root(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            copied_skill_root = temp_root / "research-toolkit"
            ...
            env = os.environ.copy()
            env["RTK_SKIP_SYSTEM_INSTALL"] = "1"
            env["RTK_SKIP_PIP_SYNC"] = "1"
            env["RTK_REQUIREMENTS_FILE"] = str(temp_root / "missing-requirements.txt")
            env["RTK_MANAGED_ROOT"] = str(temp_root / ".managed-runtime")
            env["RTK_FFMPEG_BIN"] = str(fake_ffmpeg)
            env["PYTHONPATH"] = (
            ...
            self.assertFalse((Path(env["RTK_MANAGED_ROOT"]) / "requirements.sha256").exists())
            self.assertTrue((Path(env["RTK_MANAGED_ROOT"]) / "logs" / "doctor.json").exists())
