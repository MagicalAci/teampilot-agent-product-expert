...
from rtk.config import PLATFORM_BACKEND_PRIORITY, PLATFORM_OUTPUT_ORDER, PLATFORM_SAMPLE_TARGETS
from rtk.utils import ensure_dir, now_compact, now_iso, read_json, slugify, write_json, write_text


# ── LRS Component Infrastructure (ported from local-research-system) ─────────

SKILL_ROOT = Path(__file__).resolve().parents[2]
ASSETS_ROOT = SKILL_ROOT / "assets"

DEFAULT_RUNTIME_HOME = Path(
    os.environ.get("LOCAL_RESEARCH_HOME", "~/.local/share/cursor-research")
).expanduser()
LOCAL_ENV_PATH = DEFAULT_RUNTIME_HOME / ".env.research-toolkit"
LOCAL_ENV_EXAMPLE_PATH = DEFAULT_RUNTIME_HOME / ".env.research-toolkit.example"
AUTH_ROOT = DEFAULT_RUNTIME_HOME / "auth"
DEFAULT_VENDOR_ROOT = DEFAULT_RUNTIME_HOME / "vendor"
DEFAULT_CRAWLERS_ROOT = Path(
    os.environ.get("LOCAL_CRAWLERS_ROOT", "~/crawlers")
).expanduser()
...
def load_local_env_file(path: Path = LOCAL_ENV_PATH) -> list[str]:
    loaded: list[str] = []
    if not path.exists():
        return loaded
...
def upsert_local_env(entries: dict[str, str], path: Path = LOCAL_ENV_PATH) -> Path:
    current = read_env_entries(path)
    current.update({key: value for key, value in entries.items() if value})
    write_env_entries(path, current)
...
DEFAULT_DEERFLOW_ROOT = Path(
    os.environ.get("DEERFLOW_ROOT", str(DEFAULT_VENDOR_ROOT / "deer-flow"))
).expanduser()
DEFAULT_MEDIACRAWLER_ROOT = Path(
    os.environ.get("MEDIACRAWLER_ROOT", str(DEFAULT_CRAWLERS_ROOT / "MediaCrawler"))
).expanduser()
DEFAULT_XHS_DOWNLOADER_ROOT = Path(
    os.environ.get("XHS_DOWNLOADER_ROOT", str(DEFAULT_CRAWLERS_ROOT / "XHS-Downloader"))
).expanduser()

COMPONENT_SPECS = {
    "deerflow": {
        "display_name": "DeerFlow",
        "env_var": "DEERFLOW_ROOT",
        "default_path": DEFAULT_DEERFLOW_ROOT,
        "repo_url": "https://github.com/bytedance/deer-flow.git",
        "markers": ("backend/pyproject.toml", "Makefile"),
        "aliases": (
            "~/deer-flow",
            "~/deerflow",
            "~/development/deer-flow",
            "~/Downloads/deer-flow",
        ),
    },
    "mediacrawler": {
        "display_name": "MediaCrawler",
        "env_var": "MEDIACRAWLER_ROOT",
        "default_path": DEFAULT_MEDIACRAWLER_ROOT,
        "repo_url": "https://github.com/NanmiCoder/MediaCrawler.git",
        "markers": ("pyproject.toml", "README_en.md"),
        "aliases": (
            "~/crawlers/MediaCrawler",
            "~/.local/share/cursor-research/vendor/MediaCrawler",
        ),
    },
    "xhs_downloader": {
        "display_name": "XHS-Downloader",
        "env_var": "XHS_DOWNLOADER_ROOT",
        "default_path": DEFAULT_XHS_DOWNLOADER_ROOT,
        "repo_url": "https://github.com/JoeanAmier/XHS-Downloader.git",
        "markers": ("README_EN.md", "main.py"),
        "aliases": (
            "~/crawlers/XHS-Downloader",
            "~/.local/share/cursor-research/vendor/XHS-Downloader",
        ),
    },
}
...
def read_json_asset(name: str) -> dict:
    return json.loads(read_text_asset(name))
...
def candidate_paths(component: str) -> list[Path]:
    spec = COMPONENT_SPECS[component]
    paths: list[Path] = []
    env_value = os.environ.get(spec["env_var"], "").strip()
    if env_value:
        paths.append(Path(env_value).expanduser())
    paths.append(spec["default_path"])
    for alias in spec["aliases"]:
        paths.append(Path(alias).expanduser())
    return unique_paths(paths)


def is_valid_component_path(component: str, path: Path) -> bool:
    if not path.exists():
        return False
    markers = COMPONENT_SPECS[component]["markers"]
    return all((path / marker).exists() for marker in markers)


def resolve_component_path(component: str) -> Path | None:
    spec = COMPONENT_SPECS[component]
    env_value = os.environ.get(spec["env_var"], "").strip()
    if env_value:
        explicit_path = Path(env_value).expanduser()
        if is_valid_component_path(component, explicit_path):
            return explicit_path
        return None
    for path in candidate_paths(component):
        if is_valid_component_path(component, path):
            return path
    return None


def command_preview(command: list[str], cwd: Path | None = None) -> str:
    preview = " ".join(shlex.quote(part) for part in command)
    if cwd is None:
        return preview
    return f"cd {shlex.quote(str(cwd))} && {preview}"


# ── RTK Workspace Infrastructure ────────────────────────────────────────────
