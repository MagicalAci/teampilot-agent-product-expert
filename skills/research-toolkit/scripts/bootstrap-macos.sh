...
MANAGED_ROOT="${RTK_MANAGED_ROOT:-${HOME}/.cursor/skills-runtime/research-toolkit}"
VENV_DIR="${MANAGED_ROOT}/venv"
LOG_DIR="${MANAGED_ROOT}/logs"
REQUIREMENTS_FILE="${RTK_REQUIREMENTS_FILE:-${SKILL_ROOT}/requirements.txt}"
REQUIREMENTS_HASH_FILE="${MANAGED_ROOT}/requirements.sha256"
PYTHON_FORMULA="${RTK_PYTHON_FORMULA:-python@3.11}"
FFMPEG_FORMULA="${RTK_FFMPEG_FORMULA:-ffmpeg}"
...
Environment overrides:
  RTK_MANAGED_ROOT       Override managed runtime root.
  RTK_PYTHON_FORMULA     Override Homebrew Python formula.
  RTK_FFMPEG_FORMULA     Override Homebrew ffmpeg formula.
  RTK_FFMPEG_BIN         Override ffmpeg binary used by extract-frames.
  RTK_REQUIREMENTS_FILE  Override requirements.txt path.
  RTK_SKIP_PIP_SYNC      Set to 1 to skip pip install and hash sync.
  RTK_SKIP_SYSTEM_INSTALL Set to 1 to disable Homebrew auto-install.
EOF
}
...
ensure_homebrew() {
  ...
  if [[ "${RTK_SKIP_SYSTEM_INSTALL:-0}" == "1" ]]; then
    die "当前未找到 Homebrew，且 RTK_SKIP_SYSTEM_INSTALL=1，无法自动安装。"
  fi
...
resolve_ffmpeg_bin() {
  local brew_prefix
  if [[ -n "${RTK_FFMPEG_BIN:-}" && -x "${RTK_FFMPEG_BIN}" ]]; then
    printf '%s\n' "${RTK_FFMPEG_BIN}"
    return 0
  fi
...
ensure_ffmpeg() {
  ...
  export RTK_FFMPEG_BIN="${ffmpeg_bin}"
}
