...
description: 统一调研分析工具包 — 支持单产品深度分析、方向调研、市场全景、用户研究等多种任务类型
version: 1.0.0
commands:
  - /深度调研
  - /爬取
...
- **工具**：DeerFlow 深度研究、MediaCrawler/XHS-Downloader 社媒采集、脚本归档统计
...
默认平台：`web` / `appstore` / `xiaohongshu` / `weibo` / `bilibili` / `zhihu` / `seo` / `pricing`
