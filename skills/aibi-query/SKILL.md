...
| 数据库 | 业务 | 表数量 | 线上数据 | 备注 |
|--------|------|--------|----------|------|
| **sparklab_starcard** | 星卡（追星小卡鉴定 + 站姐活动电商） | 26 | ✅ 有 | UAT 即线上环境，有真实业务数据 |
| **sparklab_picbook** | 绘本（AI 绘本阅读硬件 + 语音合成） | 19 | ✅ 有 | UAT 即线上环境，有真实业务数据 |
| **sg_sparklab_poptoy** | 泡泡玩具（AI 玩偶社区 + 聊天 + IAP） | 18 | ✅ 有 | UAT 即线上环境，有真实业务数据 |
...
| 400 "连接异常" | 该库在当前环境无实例 | 确认库名拼写正确，联系 DBA 检查实例状态 |
...
| 返回空数据 | 查询条件过滤了所有行 | 放宽时间范围或检查 WHERE 条件 |
