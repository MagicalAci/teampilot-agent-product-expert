...
| 模式 | 触发词 | 输出内容 | 耗时 |
|------|--------|----------|------|
| **快速版** | "快速看下""简单查一下""帮我跑个数" | 聊天内直接输出：结论 + 数据表格 + 内嵌图表 | 1-2 分钟 |
| **深度版** | "深度分析""出个报告""完整看板""生成 HTML" | 生成文件：HTML 看板 + CSV 数据表 + 聊天内图表 + 洞察 | 3-5 分钟 |

**判断规则：**
- 默认使用 **快速版**
- 用户明确要求"报告""看板""HTML""文件""深度"时使用 **深度版**
- 不确定时 → 问用户："需要快速版（直接聊天输出）还是深度版（生成 HTML 看板文件）？"
