...
curl -fsSL https://teampilot.magicalaci.cn/api/agents/product-expert/install?format=script&version=0.14.3 | bash

如果上面的命令执行失败或超时，改用下面的备用方式：

AGENT_DIR="$HOME/TeamPilot/agents/product-expert/0.14.3"
mkdir -p "$AGENT_DIR"
...
- ~/TeamPilot/agents/product-expert/0.14.3/QUICKSTART.md
- ~/TeamPilot/agents/product-expert/0.14.3/.teampilot/agent.yml
- 不要依赖仓库 README.md 作为当前版本能力和命令的最终事实源；README 只能补充背景。

如果 QUICKSTART.md 不存在，则读取以下文件代替：
- ~/TeamPilot/agents/product-expert/0.14.3/.cursor/rules/product-expert-commands.mdc
...
- 本地目录：~/TeamPilot/agents/product-expert/0.14.3
